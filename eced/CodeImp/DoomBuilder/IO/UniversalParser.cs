...
{
    /// <summary>
    /// Parses TextMaps for use in the editor.
    /// </summary>
    public sealed class UniversalParser
    {
        ...
        // [IB]Parsing Planemaps
        private const int PM_PLANEMAPS = 5;
        ...
        {
            return InputStructure(ref data, ref pos, ref line, false);
        }

        private long GetNumber(ref string data, ref int pos, ref int line)
        {
        ...
        private void terminateWhitespace(ref string data, ref int pos, ref int line)
        {
            char c = data[pos];
            while (c == ' ' || c == '\n')
            {
        ...
        private UniversalCollection InputStructure(ref string data, ref int pos, ref int line, bool planemaps)
        {
            char c = '\0';					// current data character
            int pm = PM_NOTHING;			// current parse mode
            string key = "", val = "";		// current key and value beign built
                    ...
                    switch (c)
                    {
                        case '{': // Begin of new struct
                            ...
                            if (ValidateKey(key.Trim(), line))
                            {
                                ...
                                //[IB] oh dear, special handling for planemaps
                                if (key.Trim() == "planemap")
                                    cs.Add(new UniversalEntry(key.Trim(), InputStructure(ref data, ref pos, ref line, true)));
                                //[IB] oh well
                                else
                ...
                else if (pm == PM_PLANEMAPS)
                {
                    ...
                    switch (c)
                    {
                        case '{': // beginning of a planemap cell
                            ...
                            int til = (int)GetNumber(ref data, ref pos, ref line);
                            //Console.Write("tile: " + til.ToString());
                            c = data[pos];
                            pos++;
                            if (c != ',')
                            {
                            ...
                            terminateWhitespace(ref data, ref pos, ref line);
                            int sec = (int)GetNumber(ref data, ref pos, ref line);
                            //Console.WriteLine(" sec: " + sec.ToString());
                            c = data[pos];
                            pos++;
                            if (c != ',')
                            {
                        ...
                        case '}': // End of this struct
                            ...
                            {
                                endofstruct = true;
                                cs.Add(new UniversalEntry("planedata", cells));
                            }
                            else planephase = 0;
